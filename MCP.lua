...
-- Function to update frame scale based on UI scale
function MCP_UpdateFrameScale()
	if MCP_AddonList then
		local uiScale = GetCVar("uiScale");
		if uiScale and uiScale ~= "" then
			MCP_AddonList:SetScale(tonumber(uiScale));
		else
			-- Default to 1.0 if uiScale is not available
			MCP_AddonList:SetScale(1.0);
		end
	end
end
...
function MCP_OnLoad()
	...
	LoadAddOn = MCP_new_LoadAddOn;
        ...
        -- Register event to listen for UI scale changes
        this:RegisterEvent("CVAR_UPDATE");
        this:RegisterEvent("ADDON_LOADED");
end

-- Event handler for UI scale changes and addon loading
function MCP_OnEvent(event)
	if event == "CVAR_UPDATE" then
		-- Check if the UI scale has changed and update frame scale accordingly
		MCP_UpdateFrameScale();
	elseif event == "ADDON_LOADED" and arg1 == "MCP" then
		-- Set initial scale when addon loads
		MCP_UpdateFrameScale();
	end
end
...
function MCP_SlashHandler(msg)
        ...
        -- Update scale when showing the panel
        MCP_UpdateFrameScale();
end
...
function MCP_AddonList_OnShow()
	...
	-- Update frame scale when showing
	MCP_UpdateFrameScale();
